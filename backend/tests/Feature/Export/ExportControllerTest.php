...
final class ExportControllerTest extends TestCase
{
    ...
    {
        ItineraryItem::factory()->count(3)->create(['user_id' => $this->user->id]);

        $response = $this->actingAs($this->user)
            ->postJson('/api/export/itinerary-pdf');
    ...
    {
        $response = $this->postJson('/api/export/itinerary-pdf');
    ...
    {
        Photo::factory()->count(2)->create(['user_id' => $this->user->id]);

        $response = $this->actingAs($this->user)
            ->postJson('/api/export/photobook-pdf');
    ...
    {
        $response = $this->postJson('/api/export/photobook-pdf');
    ...
    {
        $response = $this->actingAs($this->user)
            ->postJson('/api/export/slideshow-video');
    ...
    {
        $response = $this->postJson('/api/export/slideshow-video');
    ...
    public function test_zip_returns_zip_binary(): void
    {
        ...
        ItineraryItem::factory()->count(2)->create(['user_id' => $this->user->id]);
    ...
    {
        $response = $this->postJson('/api/export/zip');
