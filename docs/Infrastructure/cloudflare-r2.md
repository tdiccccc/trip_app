...
| R2 バケット名 | `trip-app-photos` | 写真の保存先 |
...
# バケット作成
wrangler r2 bucket create trip-app-photos
...
| 項目 | 設定値 |
|------|--------|
| Token name | `trip-app-laravel` |
| Permissions | **Object Read & Write** |
| Specify bucket(s) | 「Apply to specific buckets only」> `trip-app-photos` を選択 |
| TTL | (任意) 無期限でも可 |
...
1. ダッシュボード > R2 > `trip-app-photos` バケット
2. 「Settings」タブ
...
1. ダッシュボード > R2 > `trip-app-photos` バケット > 「Settings」
...
[
  {
    "AllowedOrigins": [
      "https://trip-app-frontend-xxx.run.app",
      "http://localhost:3900"
    ],
...
| `AWS_BUCKET` | `trip-app-photos` | バケット名 |
