...
|------------|---------|--------|-----|------------------|------------------|
| `trip-app-backend` | `trip-app/backend` | 512Mi | 1 | 0 | 1 |
| `trip-app-frontend` | `trip-app/frontend` | 256Mi | 1 | 0 | 1 |
...
# イメージビルド
docker build \
  -t asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip-app/backend:initial \
  -f backend/Dockerfile \
  backend/

# イメージプッシュ
docker push asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip-app/backend:initial

# Cloud Run サービス作成
gcloud run deploy trip-app-backend \
  --image=asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip-app/backend:initial \
...
# イメージビルド
docker build \
  -t asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip-app/frontend:initial \
  -f frontend/Dockerfile \
  frontend/

# イメージプッシュ
docker push asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip-app/frontend:initial

# Cloud Run サービス作成
gcloud run deploy trip-app-frontend \
  --image=asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip-app/frontend:initial \
  --region=asia-northeast1 \
  --min-instances=0 \
  --max-instances=1 \
  --memory=256Mi \
...
```bash
# Backend の URL を取得
gcloud run services describe trip-app-backend \
  --region=asia-northeast1 \
  --format="value(status.url)"

# Frontend の URL を取得
gcloud run services describe trip-app-frontend \
  --region=asia-northeast1 \
  --format="value(status.url)"
```

出力例:
- Backend: `https://trip-app-backend-xxxxx-an.a.run.app`
- Frontend: `https://trip-app-frontend-xxxxx-an.a.run.app`
...
| `APP_URL` | Yes | `https://trip-app-backend-xxx.run.app` | バックエンド自身の URL |
...
| `SANCTUM_STATEFUL_DOMAINS` | Yes | `trip-app-frontend-xxx.run.app` | Sanctum が Cookie 認証を許可するドメイン |
| `FRONTEND_URL` | Yes | `https://trip-app-frontend-xxx.run.app` | CORS 許可オリジン |
...
| `AWS_BUCKET` | Yes | `trip-app-photos` | R2 バケット名 |
...
| `NUXT_PUBLIC_API_BASE` | Yes | `https://trip-app-backend-xxx.run.app` | Laravel API のベース URL |

### 5. ヘルスチェックの確認

```bash
# Backend
curl -s https://trip-app-backend-xxx.run.app/api/health

# Frontend
curl -s https://trip-app-frontend-xxx.run.app/
```
