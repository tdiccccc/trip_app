...
| 情報             | 例                | 用途                                       |
| ---------------- | ----------------- | ------------------------------------------ |
| プロジェクト ID  | `trip-app`        | 全 GCP リソースの識別子                    |
...
```bash
# プロジェクト作成
gcloud projects create trip-app --name="Trip App"

# 作成したプロジェクトをデフォルトに設定
gcloud config set project trip-app
```

> **プロジェクト ID のルール**:
> - グローバルで一意である必要がある
> - 6〜30文字、小文字・数字・ハイフンのみ
> - `trip-app` が既に使われている場合は `trip-app-2026` 等に変更
...
# プロジェクトに課金アカウントを紐付け
gcloud billing projects link trip-app \
...
```
メモ
To set up Integration Connectors in your Google Cloud project, enable the following APIs.

Secret Manager API Not enabled
An API that enables you to store, access, and manage sensitive configuration data that are required by an application during runtime.
Connectors API Not enabled
An API that enables you to create and manage connectors to Google Cloud services and other business applications using the Integration Connectors interface.
```
...
```bash
gcloud artifacts repositories create trip \
  --repository-format=docker \
  --location=asia-northeast1 \
  --description="Trip App Docker images"
```
...
> **イメージパスの形式**:
> `asia-northeast1-docker.pkg.dev/<PROJECT_ID>/trip/<IMAGE_NAME>:<TAG>`
>
> 例: `asia-northeast1-docker.pkg.dev/trip-app/trip/backend:latest`
...
- [ ] 課金が有効化済み
- [ ] 必要な API が全て有効化済み
- [ ] Artifact Registry リポジトリ `trip` が作成済み
